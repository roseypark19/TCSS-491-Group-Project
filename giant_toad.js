class GiantToad {
    ...
    constructor(game, x, y) {
        ...
        this.spritesheet = ASSET_MANAGER.getAsset("./sprites/enemies/giant_toad.png");
                ...
                if (entity instanceof Hero) {
                    ...
                    if (dist <= this.visionDistance && !PARAMS.GAMEOVER) {
                        ...
                        if (dist <= this.attackDistance && this.frozenTimer === 0) {       
                            let flyVector = this.confusedTimer === 0 ? { x: heroCenter.x - this.BB.center.x, y: heroCenter.y - this.BB.center.y } :
                                                                         this.confusionUnitVector;
                            let arrowTheta = Math.atan2(flyVector.y, flyVector.x);
                            if (arrowTheta < 0) {
                                ...
                                if (this.shootFlag) {
                                    ...
                                    for (let i = initTheta; i < initTheta + 2 * Math.PI; i += Math.PI / 3) {
                                        this.game.addEntity(new Projectile(this.game, 
                                            ...
                                            this.BB.center.y - 16 * PARAMS.PROJECTILE_SCALE + 4 * Math.sin(i) * PARAMS.SCALE, i, false, 14, this.BB.center, 90, PARAMS.PROJECTILE_SCALE, 1));
                                    }
                                }
                            }
