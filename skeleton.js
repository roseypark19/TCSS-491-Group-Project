class Skeleton {
    ...
    constructor(game, x, y) {
        ...
        this.spritesheet = ASSET_MANAGER.getAsset("./sprites/enemies/skeleton.png");
    ...
    update() {
        ...
        if (this.state !== 6) {
            this.game.projectileEntities.forEach(entity => {
                if (entity.friendlyProjectile && this.hitBB.collide(entity.hitBB) && 
                    (!(entity.passable || entity.removeFromWorld) || (entity.passable && !(this.shotsTaken.includes(entity.id)))) && this.state !== 6) {
                    if (entity.passable) {
        ...
        if (this.state !== 6 && this.burningTimer > 0 && this.burnDamageTimer === 0 && !PARAMS.GAMEOVER) {
            this.burnDamageTimer = 1 - this.game.clockTick;
            this.hp -= 25;
            // play damaged sound
    ...
    draw(ctx) {
        this.animations[this.state].drawFrame(
            this.frozenTimer > 0 && this.damagedTimer === 0 && this.state !== 6 ? 0 : this.game.clockTick, ctx, this.x - this.game.camera.x, this.y - this.game.camera.y, PARAMS.SCALE, this.facing[0], this.facing[1]);
